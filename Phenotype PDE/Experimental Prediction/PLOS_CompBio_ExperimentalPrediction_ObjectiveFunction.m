...
% no treatment applied. Use to fit D_a = D_b, R_a

% 

function [Obj] = PLOS_CompBio_ExperimentalPrediction_ObjectiveFunction(X,D,TimeVec);
 
UntreatedPopulationData = D(1:4); % WildTypePopulationData;
TreatedPopulationData = D([1,2,5:length(D)]); 
 
WildTypeTime =  TimeVec;  
...
EvalSol1 = deval(sol1,WildTypeTime(1:4),1)+ deval(sol1,WildTypeTime(1:4),2) ;

% %% Solve the Treated ODE system
 [sol1Treated] = AdaptiveSizeTreatedTimeDynamicsTreated(TotalTime,TreatCapacityIC,PA);
 EvalSol1Treated = deval(sol1Treated,WildTypeTime([3:end]),1)+ deval(sol1Treated,WildTypeTime([3:end]),2) ; %Calculate the number of cells at each time point.

%% Calculate the Objective function 
 Obj =  sqrt( sum( (log(EvalSol1)./log(10) - log(UntreatedPopulationData)./log(10) ).^2 ) + sum( (log(EvalSol1Treated)./log(10) - log(TreatedPopulationData([3:end]))./log(10) ).^2 ) )  ; % enforce two penalties: Ra>Rb and Ra < DaMax ;
