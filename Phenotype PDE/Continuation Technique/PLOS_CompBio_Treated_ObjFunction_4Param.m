...
function [Obj] = PLOS_CompBio_Treated_ObjFunction_4Param(X,D);
...
EvalSol1 = deval(sol1,WildTypeTime,1)+ deval(sol1,WildTypeTime,2) ;

% %% Solve the Treated ODE system
 [sol1Treated] = AdaptiveSizeTreatedTimeDynamicsTreated(TotalTime,TreatCapacityIC,PA);
 EvalSol1Treated = deval(sol1Treated,WildTypeTime([3,4]),1)+ deval(sol1Treated,WildTypeTime([3,4]),2) ;
%% Calculate the Objective function 
Obj =  sqrt( sum( (log(EvalSol1)./log(10) - log(UntreatedPopulationData)./log(10) ).^2 ) + sum( (log(EvalSol1Treated)./log(10) - log(TreatedPopulationData([3,4]))./log(10) ).^2 ) )  ; % enforce two penalties: Ra>Rb and Ra < DaMax ;
